...
function yk_mt_shortcode_meal_tracker( $user_defined_arguments ) {
	...
	if ( 'settings' === $shortcode_mode ) {
		...
		$html .= sprintf( '<br />
			...
			esc_html__( 'Settings', YK_MT_SLUG )
		);
...
function yk_mt_shortcode_meal_tracker_navigation( $todays_entry_id = NULL ) {
	...
	$html .= sprintf(
		...
		esc_html__( 'Select a date', YK_MT_SLUG )
	);
...
function yk_mt_shortcode_meal_tracker_summary( $arguments ) {
	...
	return '
		<div class="yk-mt__summary">
			<div class="yk-mt__summary-title"><span class="fa fa-check-circle"></span>' . esc_html__( 'Today\'s summary', YK_MT_SLUG ) . '</div>
...
function yk_mt_shortcode_meal_tracker_settings( $target = NULL ) {
	...
	if ( true === $target_missing ) {
		...
		$html .= yk_mt_html_accordion_section( [ 'id' => 0,
			'title' => esc_html__( 'Getting started', YK_MT_SLUG ),
			'content' => '<p>' . esc_html__( 'Before you can start recording your calorie intake, you must set your calorie allowance for the day. You can achieve this by completing the following form.', YK_MT_SLUG ) . '</p>',
			'is-active' => true
		]);
	...
	if ( false === $target_missing ) {
		$calories_html .= '<p>' . sprintf(  esc_html__( 'Your current daily allowance is: %1$dkcal.', YK_MT_SLUG ), $target ) . '</p>';
	}

	if ( true === empty( $calorie_sources ) ) {

		$calories_html .= '<p>' . esc_html__( 'Your calorie allowance has been set by an administrator and can not be changed.', YK_MT_SLUG ) . '</p>';
		...
		if ( yk_mt_wlt_enabled_for_mt() &&
			...
			$calories_html .= '<p class="yk-mt-warning">' . esc_html__( '<strong>Please note</strong>: If you wish to specify Weight Tracker as your target source, then you must complete your Weight Tracker profile (e.g. height, DoB, etc) so your suggested calorie intake can be calculated.', YK_MT_SLUG ) . '</p>';
			$keys_to_disable = [ 'wlt' ];
		}

		$calories_html .= yk_mt_form_select( esc_html__( 'Calorie target source', YK_MT_SLUG ),
											'calorie-source',
		...
		$calories_html .= yk_mt_form_number( esc_html__( 'Specify your own target: ', YK_MT_SLUG ),
			'allowed-calories',
		...
		$calories_html .= sprintf( ' <p class="yk-mt-info yk-mt-hide-if-adding">%1$s</p>',
			esc_html__( 'Changes to these settings will adjust today\'s entry and future entries. Historic entries shall not be changed.', YK_MT_SLUG )
		);
	}

	$html .= yk_mt_html_accordion_section( [    'id' => 1,
												'title' => esc_html__( 'Calorie Intake', YK_MT_SLUG ),
	...
	$html .= sprintf( '<br /><button id="yk-mt-button-save-settings" class="yk-mt__btn yk-mt__btn--medium">%1$s</button>
		...
		esc_html__( 'Save Settings', YK_MT_SLUG ),
		yk_mt_shortcode_get_current_url(),
		esc_html__( 'Cancel', YK_MT_SLUG )
	);
...
function yk_mt_shortcode_meal_tracker_meal_types() {
	...
	if ( false === empty( $meal_types ) ) {
		...
		foreach ( $meal_types as $meal_type ) {
			...
			$meal_type_html = sprintf( '<div class="yk-mt__btn-wrap">%s</div>', yk_mt_shortcode_meal_tracker_add_meal_button( esc_html__( 'Add Meal', YK_MT_SLUG ), $meal_type['id'] ) );
...
function yk_mt_shortcode_meal_tracker_add_meal_dialog() {
	...
	$html = sprintf( '
		...
		',
		$top,
		esc_html__( 'Close', YK_MT_SLUG ),
		esc_html__( 'Log a meal', YK_MT_SLUG ),
		esc_html__( 'Edit meal', YK_MT_SLUG )
	);

	// Build HTML for "Add Meal" tab
	$add_form = yk_mt_shortcode_meal_tracker_select( 'yk-mt-meal-id', esc_html__( 'Search for a meal', YK_MT_SLUG ) );
	...
	if ( false === empty( $add_form ) ) {
		...
		$html .= sprintf('  	<h4 class="yk-mt__modal-subtitle">%1$s</h4>
									<p>%2$s:</p>',
									esc_html__( 'Search your existing meal collection', YK_MT_SLUG ),
									esc_html__( 'Search and select a meal from your existing collection', YK_MT_SLUG )
		);
		...
		$html .= sprintf(
			...
			esc_html__( 'How many have you had?', YK_MT_SLUG )
		);
	...
	$html .= sprintf('
		...
		esc_html__( 'Close screen after adding meal(s)', YK_MT_SLUG )
	);
	...
	if ( true === yk_mt_license_is_premium() || yk_mt_meal_count() < 40 ) {
		...
		$html .= sprintf( '<br /><p>%1$s</p>', esc_html__( 'Unfortunately you have reached the maximum of 40 meals and are unable to add anymore.', YK_MT_SLUG ) );
	}
...
function yk_mt_shortcode_meal_tracker_add_new_meal_form_navigation() {
	...
	$html = sprintf( '  	<h4 id="yk-mt-header-meal-add" class="yk-mt__modal-subtitle yk-mt-hide-if-editing">%1$s</h4>
							...
							esc_html__( 'Add a new meal to your collection', YK_MT_SLUG )
	);
	...
	$html .= sprintf( '		<button id="yk-mt-button-meal-nav-manually-add" class="yk-mt__btn yk-mt__btn--medium" title="%1$s">
		...
		esc_html__( 'Manually', YK_MT_SLUG )
	);
...
function yk_mt_shortcode_meal_tracker_manual_meal_entry_form( $previous_values = [] ) {
	...
	$html .= yk_mt_form_text( esc_html__( 'Name', YK_MT_SLUG ),	'add-meal-name', $previous_values[ 'name' ] );

	$html .= yk_mt_form_text( esc_html__( 'Description', YK_MT_SLUG ), 'add-meal-description', $previous_values[ 'description' ], 200, false );

	$html .= yk_mt_form_number( esc_html__( 'Calories', YK_MT_SLUG ), 'add-meal-calories', $previous_values[ 'calories' ], '', 1,0 );
	...
	if ( false === is_admin() ) {
		...
		$html .= sprintf( '<p class="yk-mt__info-box yk-mt-info yk-mt-hide-if-adding">
							...
							esc_html__( 'Today\'s calorie count shall be adjusted if a meal\'s calorific value is modified. Other entries will only be re-counted if done manually.', YK_MT_SLUG )
		);
	}
	...
	$html .= yk_mt_form_select( esc_html__( 'Unit', YK_MT_SLUG ), 'add-meal-unit', $previous_values[ 'unit' ], yk_mt_units() );

	$html .= yk_mt_form_number( esc_html__( 'Quantity', YK_MT_SLUG ), 'add-meal-quantity', $previous_values[ 'quantity' ], '', 1, 1, 99999, true, false, true );
...
function yk_mt_shortcode_meal_tracker_add_new_meal_external_form() {
	...
	$html .= yk_mt_shortcode_meal_tracker_select( 'yk-mt-search-external', esc_html__( 'Search for a meal', YK_MT_SLUG ) );
	...
	$html .= sprintf( '
		<div class="yk-mt-add-new-meal-form-search-external" style="display: none">
			...
			',
		esc_html__( 'Cancel', YK_MT_SLUG ),
		esc_html__( 'Add', YK_MT_SLUG ),
		esc_html__( 'Add and Return', YK_MT_SLUG )
	);
...
function yk_mt_shortcode_meal_tracker_add_new_meal_servings() {
	...
	$html .= sprintf( '<h3 class="yk-mt__modal-subtitle yk-mt-hide-if-editing">%1$s</h3>', esc_html__( 'Select a serving size', YK_MT_SLUG ) );

	$html .= yk_mt_shortcode_meal_tracker_select( 'yk-mt-search-external-servings', esc_html__( 'Select a serving size', YK_MT_SLUG ) );
