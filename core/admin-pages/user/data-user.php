...
?>
	<div class="postbox <?php yk_mt_postbox_classes( 'chart' ); ?>" id="chart">
		<?php yk_mt_postbox_header( [ 'title' => esc_html__( 'Chart', 'meal-tracker' ), 'postbox-id' => 'chart', 'postbox-col' => 'yk-mt-user-data-one' ] ); ?>
		<div class="inside">
			<?php
				...
				if ( false === empty( $entries ) ) {
					echo yk_mt_chart_line_allowed_versus_used( [
						'entries'   => $entries,
						'max'       => 15,
						'title'     => esc_html__( 'Latest 15 entries', 'meal-tracker' )
					]);
				} else {
					printf ( '<p><em>%s</em></p>', esc_html__( 'No results', 'meal-tracker' ) );
				}
			?>
		</div>
	</div>
...
?>
	<div class="postbox <?php yk_mt_postbox_classes( 'table' ); ?>" id="table">
		<?php yk_mt_postbox_header( [ 'title' => esc_html__( 'Entries for this user', 'meal-tracker' ), 'postbox-id' => 'table', 'postbox-col' => 'yk-mt-user-data-one' ] ); ?>
		<div class="inside">
			<?php
