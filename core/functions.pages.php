...
function yk_mt_user_side_bar_postbox_delete_cache( $user_id ) {
	...
	<div class="postbox <?php yk_mt_postbox_classes( 'delete-cache' ); ?>" id="delete-cache">
		<?php yk_mt_postbox_header( [ 'title' => esc_html__( 'Delete cache for user', 'meal-tracker' ), 'postbox-id' => 'delete-cache', 'postbox-col' => 'yk-mt-user-data-one-sidebar' ] ); ?>
		<div class="inside">
			<a href="<?php echo esc_url( yk_mt_link_admin_page_user($user_id, 'user', [ 'delete-cache' => 'y' ] ) );?>" class="button-secondary"><?php echo esc_html__( 'Delete Cache', 'meal-tracker' ); ?></span></a>
		</div>
	</div>
...
function yk_mt_user_side_bar_postbox_delete_data( $user_id ) {
	...
	<div class="postbox <?php yk_mt_postbox_classes( 'delete-data' ); ?>" id="delete-data">
		<?php yk_mt_postbox_header( [ 'title' => esc_html__( 'Delete user data', 'meal-tracker' ), 'postbox-id' => 'delete-data', 'postbox-col' => 'yk-mt-user-data-one-sidebar' ] ); ?>
		<div class="inside">
			<a href="<?php echo esc_url( yk_mt_link_admin_page_user($user_id, 'user', [ 'delete-entries' => 'y' ] ) );?>" class="button-secondary yk-mt-button-confirm"><?php echo esc_html__( 'All Entries', 'meal-tracker' ); ?></span></a>
			<a href="<?php echo esc_url( yk_mt_link_admin_page_user($user_id, 'user', [ 'delete-meals' => 'y' ] ) );?>"
			   data-content="<?php echo esc_html__( 'All of the user\'s meals will be marked as deleted. They will still reside in the system, yet can only be viewed against old entries. They can no longer be added to entries', 'meal-tracker' ); ?>"
			   class="button-secondary yk-mt-button-confirm"><?php echo esc_html__( 'All Meals', 'meal-tracker' ); ?></span></a>
		</div>
	</div>
...
function yk_mt_user_side_bar_postbox_allowed_sources( $user_id ) {
	...
	<div class="postbox <?php yk_mt_postbox_classes( 'allowed-calories' ); ?>" id="allowed-calories">
		<?php yk_mt_postbox_header( [ 'title' => esc_html__( 'Allowed calories', 'meal-tracker' ), 'postbox-id' => 'allowed-calories', 'postbox-col' => 'yk-mt-user-data-one-sidebar' ] ); ?>
		<div class="inside">
			<p><?php echo esc_html__( 'When a new entry is created for this user, their allowed calories will be set in the following way', 'meal-tracker' ); ?>:</p>
			<?php
			...
			<table class="yk-mt-sidebar-stats">
				<tr>
					<th colspan="2"><?php echo esc_html__( 'Source', 'meal-tracker' ); ?></th>
				</tr>
				<tr>
					<td colspan="2">
						<?php
						...
						if ( false === empty( $sources ) ) {
							...
							printf( '<p class="yk-mt-error">%s</p>', esc_html__( 'You must specify one or more calorie sources in settings.', 'meal-tracker' ) );
						}
						?>
					</td>
				</tr>
                <?php if ( ! empty( $selected_source[ 'value' ] ) ) : ?>
                    <tr>
                        ...
                        <td><?php ws_ls_echo( yk_mt_format_calories( $selected_source[ 'value' ] ) ); ?></td>
                    </tr>
                <?php endif; ?>
			</table>
			<?php
			...
			<?php if ( true === YK_MT_IS_PREMIUM &&
				...
				<form class="yk-mt-admin-form yk-mt-side-bar-admin-allowance<?php echo ( 'admin' !== $selected_source[ 'key' ] ) ? ' yk-mt-hide' : ''; ?>" id="yk-mt-admin-allowance" method="post" action="<?php echo esc_url( yk_mt_link_current_url() ); ?>">
					...
					echo yk_mt_form_number( esc_html__( 'Set Target: ', 'meal-tracker' ),
					...
					?>
					<input type="submit" class="button" value="<?php echo esc_html__( 'Save', 'meal-tracker' ); ?>" />
				</form>
			<?php endif; ?>
		</div>
	</div>
...
function yk_mt_user_side_bar_postbox_user_stats( $user_id ) {
	...
	<div class="postbox <?php yk_mt_postbox_classes( 'user-stats' ); ?>" id="user-stats">
		<?php yk_mt_postbox_header( [ 'title' => esc_html__( 'User Information', 'meal-tracker' ), 'postbox-id' => 'user-stats', 'postbox-col' => 'yk-mt-user-data-one-sidebar' ] ); ?>
		<div class="inside">
			<table class="yk-mt-sidebar-stats">
				<tr>
...
function yk_mt_user_side_bar_postbox_entry( $entry = NULL ) {
	...
	$current_url 	= yk_mt_link_current_url();
	$title 			= ( false === empty( $_GET[ 'mode' ] ) && 'entry' === $_GET[ 'mode' ] ) ? esc_html__( 'Entry summary', 'meal-tracker' ) : esc_html__( 'Today\'s entry', 'meal-tracker' );
?>
	<div class="postbox <?php yk_mt_postbox_classes( 'entry-summary' ); ?>" id="entry-summary">
		...
		<div class="inside">
			<?php if ( NULL !== $entry ): ?>
				...
				<table class="yk-mt-sidebar-stats">
					<tr>
						<th><?php echo esc_html__( 'Date', 'meal-tracker' ); ?></th>
			...
			<?php else: ?>
				<p><?php echo esc_html__( 'No entry was found for the given date', 'meal-tracker' ); ?>.</p>
			<?php endif; ?>
		</div>
	</div>
...
?>
    ...
    ?>
     <div class="postbox">
        <h2 class="hndle"><?php echo esc_html__( 'Options', 'meal-tracker' ); ?></h2>
        <div class="inside">
        	<center>
        		<a href="<?php echo esc_url( admin_url( 'admin.php?page=yk-mt-meals&mode=meal' ) ); ?>" class="button-primary"><?php echo esc_html__( 'Add a new meal', 'meal-tracker' ); ?></span></a>
        		<a href="<?php echo esc_url( admin_url( 'admin.php?page=yk-mt-meals&mode=import' ) ); ?>" class="button-secondary"><?php echo esc_html__( 'Import from CSV', 'meal-tracker' ); ?></span></a>
			</center>
        </div>
     </div>
     <?php endif;
...
?>
	<div class="postbox <?php yk_mt_postbox_classes( 'summary-counts' ); ?>" id="summary-counts">
		<?php yk_mt_postbox_header( [ 'title' => esc_html__( 'Summary counts', 'meal-tracker' ), 'postbox-id' => 'summary-counts', 'postbox-col' => 'yk-mt-user-sidebar' ] ); ?>
        <div class="inside">
             <table class="yk-mt-sidebar-stats">
                 <tr>
...
?>
 	<div class="postbox <?php yk_mt_postbox_classes( 'user-search' ); ?>" id="user-search">
		<?php yk_mt_postbox_header( [ 'title' => esc_html__( 'User Search', 'meal-tracker' ), 'postbox-id' => 'user-search', 'postbox-col' => $column ] ); ?>
        <div class="inside">
...
function yk_mt_user_header( $user_id ) {
    ...
    printf('
        ...
        esc_html__( 'Back', 'meal-tracker' ),
        esc_url( get_edit_user_link( $user_id ) ),
        esc_html__( 'WordPress Record', 'meal-tracker' ),
...
function yk_mt_link_admin_page_meal_edit( $meal_id, $text = '', $add_back_link = true ) {
	...
	if ( true === empty( $text ) ) {
		$text = esc_html__( 'Edit Meal', 'meal-tracker' );
	}
...
function yk_mt_user_search_form() {
    ...
    ?>	<p><?php echo esc_html__( 'Enter a user\'s email, display name or username then click "Search".', 'meal-tracker' ); ?></p>
...
function yk_mt_postbox_header( $args = [] ) {
		...
		$args = wp_parse_args( $args, [		'title'			=> esc_html__( 'Title', 'meal-tracker' ),
											'show-controls' => true,
											'postbox-id'	=> NULL,
